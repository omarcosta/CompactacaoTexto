...
tag = "#"  # Tag que marca o início de uma sequência comprimida, conforme Teoria.PDF (página 11) 
caminho_arquivo_original = "repeticao/arquivo.txt"  # Caminho do arquivo que será lido
caminho_arquivo_compactado = "repeticao/compactado_arquivo.txt" # Caminho do arquivo que será gravado
...
print(f"Lido arquivo original com {len(texto_original)} caracteres.") # Mensagem de feedback - Len conta os caracteres

# 3) COMPRESSÃO DO TEXTO (Técnica: Repetição de Caracteres / Run Length)
...
while i < len(texto_original): # Loop que percorre toda a string original
    caractere_analisado = texto_original[i] # Pega o caractere que vamos começar a contar
    ...
    while j < len(texto_original) and texto_original[j] == caractere_analisado: # Continua enquanto o caractere for igual E não chegar no fim do texto
        contador += 1 # Incrementa o contador de repetições
        if contador == 9: # Verifica se atingiu o limite da Regra 12 (só conta até 9)
    ...
    # A teoria (página 11) manda comprimir apenas se for 4 ou mais
    if contador >= 4: # Se a contagem for 4 ou mais, aplica a compressão
        texto_comprimido += f"{tag}{caractere_analisado}{contador}" # Grava a tag, o caractere e o número (ex: #a9)
    else: # Se for 1, 2 ou 3 repetições
        texto_comprimido += caractere_analisado * contador # Grava os caracteres de forma literal (ex: ccc)
...
# 4) GRAVAÇÃO DO ARQUIVO COMPACTADO (Regra 5)
...
while i < len(texto_comprimido): # Loop que percorre toda a string comprimida
    ...
    if caractere_atual == tag: # Verifica se o caractere é a nossa 'tag' (ex: #) 
        # Se for a tag, sabemos que é um bloco comprimido
        caractere_analisado = texto_comprimido[i + 1] # O próximo caractere é a letra a ser repetida
        ...
        texto_descompactado += caractere_analisado * contador # Adiciona o caractere repetido 'contador' vezes
