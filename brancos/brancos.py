...
while i < len(texto_original): # Loop que percorre toda a string original
    char_base = texto_original[i] # Pega o caractere que vamos analisar
    
    if char_base == ' ': # Verifica se o caractere é um espaço
        ...
        texto_comprimido += char_base # Grava o caractere de forma literal
...
while i < len(texto_comprimido): # Loop que percorre toda a string comprimida
    ...
    if char_atual == tag: # Verifica se o caractere é a nossa 'tag' (ex: b) 
